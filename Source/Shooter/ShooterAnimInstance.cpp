...
void UShooterAnimInstance::TurnInPlace()
{
	...
	if (Speed > 0 || bIsInAir) {
		...
		TIPCharacterYawLastFrame = TIPCharacterYaw;
		TIPCharacterYaw = ShooterCharacter->GetActorRotation().Yaw;
		const double TIPYawDelta{ TIPCharacterYaw - TIPCharacterYawLastFrame };

		// Root Yaw Offset, updated and clamped to [-180, 180]
		RootYawOffset = UKismetMathLibrary::NormalizeAxis(RootYawOffset - TIPYawDelta);

		// 1.0 if turning, 0.0 if not
		const double Turning{ GetCurveValue(TEXT("Turning")) };
		if (Turning > 0) {

			bTurningInPlace = true;
			RotationCurveLastFrame = RotationCurve;
			RotationCurve = GetCurveValue(TEXT("Rotation"));
			const double DeltaRotation{ RotationCurve - RotationCurveLastFrame };

			// RootYawOffset > 0 -> Turning Left. RootYawOffset < 0 -> Turning Right
			RootYawOffset > 0 ? RootYawOffset -= DeltaRotation : RootYawOffset += DeltaRotation;

			const double ABSRootYawOffset{ FMath::Abs(RootYawOffset) };
			if (ABSRootYawOffset > 90.f) {

				const double YawExcess{ ABSRootYawOffset - 90.f };
