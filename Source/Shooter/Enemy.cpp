	...
	DeathTime(4.f)
{
	...
	// Construct right and left weapon collision boxes
	RightWeaponCollision = CreateDefaultSubobject<UBoxComponent>(TEXT("Right Weapon Box"));
	RightWeaponCollision->SetupAttachment(GetMesh(), FName("RightWeaponBone"));
	RightWeaponCollision->SetRelativeScale3D(FVector(0.38, 3, 0.8));
	RightWeaponCollision->SetRelativeLocation(FVector(0, -60, 0));

	LeftWeaponCollision = CreateDefaultSubobject<UBoxComponent>(TEXT("Left Weapon Box"));
	LeftWeaponCollision->SetupAttachment(GetMesh(), FName("LeftWeaponBone"));
	LeftWeaponCollision->SetRelativeScale3D(FVector(0.38, 3, 0.8));
	LeftWeaponCollision->SetRelativeLocation(FVector(0, 60, 0));
...
void AEnemy::PlayHitMontage(FName Section, float PlayRate)
{
	if (bCanHitReact) {
		...
		GetWorldTimerManager().SetTimer(HitReactTimer, this,
			&AEnemy::ResetHitReactTimer, HitReactTime);
	}
