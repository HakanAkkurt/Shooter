...
void AItem::ItemInterp(float DeltaTime)
{
	...
	if (Character && ItemZCurve) {
		...
		const double CurveValue = ItemZCurve->GetFloatValue(ElapsedTime);
		...
		const double DeltaZ = ItemToCamera.Size();

		const FVector CurrentLocation{ GetActorLocation() };
		// Interpolated X value
		const double InterpXValue = FMath::FInterpTo(CurrentLocation.X, CameraInterpLocation.X, DeltaTime, 30.0f);
		// Interpolated Y value
		const double InterpYValue = FMath::FInterpTo(CurrentLocation.Y, CameraInterpLocation.Y, DeltaTime, 30.0f);
		...
		if (ItemScaleCurve) {
			...
			const double ScaleCurveValue = ItemScaleCurve->GetFloatValue(ElapsedTime);
...
void AItem::StartItemCurve(AShooterCharacter* Char, bool bForcePlaySound)
{
	...
	const double CamareRotationYaw{ Character->GetFollowCamera()->GetComponentRotation().Yaw };
	// Get initial Yaw of the Item
	const double ItemRotationYaw{ GetActorRotation().Yaw };
	// Initial Yaw offset between Camera and Item
	InterpInitialYawOffset = ItemRotationYaw - CamareRotationYaw;
