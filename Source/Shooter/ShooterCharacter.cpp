	...
	bDead(false)
{
	...
	CameraBoom->TargetArmLength = 350.f; // The camera fallows at this distance behind the character
	...
	float AShooterCharacter::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInvestigator, AActor* DamageCauser)
	{
		if (Health - DamageAmount <= 0.f) {
			...
			auto EnemyController = Cast<AEnemyController>(EventInvestigator);
			if (EnemyController) {
...
void AShooterCharacter::CalculateCrosshairSpread(float DeltaTime)
{
	...
	else { // Chracter is on the ground
...
void AShooterCharacter::InterpCapsuleHalfHeight(float DeltaTime)
{
	...
	const float InterpHalfHeight{ FMath::FInterpTo(GetCapsuleComponent()->GetScaledCapsuleHalfHeight(), TargetCapsuleHalfHeight,
		DeltaTime, 20.f) };
	
	// Negative value if crouching; Positive value if standing
	const float DeltaCapsuleHalfHeigt{ InterpHalfHeight - GetCapsuleComponent()->GetScaledCapsuleHalfHeight() };
	const FVector MeshOffset{ 0.f, 0.f, -DeltaCapsuleHalfHeigt };
...
void AShooterCharacter::Tick(float DeltaTime)
{
	...
	// Calculate crosshairs spread multiplier
...
void AShooterCharacter::FinishReloading()
{
	...
	if (AmmoMap.Contains(AmmoType)) {
		...
		// Amount of ammo the Character is carrying of the EquippedWeapn type
